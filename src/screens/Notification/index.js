...
const DATA = Object.values(DATA_NOTIFICATIONS.reduce((acc, item) => {
    let AddedTime = "12"//moment(item.addedTime).format('DD-MM-YYYY') 
...
const Notifications = (props) => {
    ...
    function HandleDate(value) {
        ...
        let Day = "12"//moment(value).format('DD-MM-YYYY')
        let Today = "12"//moment(Date.now()).format('DD-MM-YYYY')
        let Yesterday = "12"//moment(date).format('DD-MM-YYYY')
    ...
    const renderItems = ({ item }) => {
        return(
            ...
            NotificationTime= {"12"}//{moment(item.addedTime).startOf('hour').fromNow()}
            />
        )
    }
